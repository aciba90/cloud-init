...
def _build_nic_order(
    ...
    taking into account `network-card-index` and `device-number` if present.

    Note that the first NIC will be the primary NIC as it will be the one with
    [network-card-index] == 0 and device-number == 0 if present.
    ...
    if macs_metadata[macs[0]].get("device-number") is None:
        # We are in a ec2-like platform without device-number info as AliYun
        return {
            mac: i
            for i, (mac, _mac_metadata) in enumerate(macs_metadata_filtered)
        }
    ...
    # Sort by (network_card_index, device_index) as some instances could have
    # multiple network cards with repeated device indexes
    return {
        mac: i
        for i, (mac, _mac_metadata) in enumerate(
            sorted(
                macs_metadata_filtered,
                key=lambda mmd: (
                    _get_key_as_int_or(
                        mmd[1], "network-card-index", float("infinity")
                    ),
                    _get_key_as_int_or(
