...
def extract_cloud_init_log(txt: bytes):
    ...
    def extract_netplan_time():
        ...
        ex_time = lambda ll: dt.datetime.strptime(
            ll.split(b" ")[1].decode(), "%H:%M:%S,%f"
        )
        t_0 = ex_time(line)
...
def main():
    ...
    for release_path in INDIR.glob("*/"):
        ...
        for boot_info_path in release_path.glob("*"):
            timestamp, base_or_update, boot_number, *_ = str(
                boot_info_path.name
            ).split("-")
            row = dict()
            row["release"] = release
            ...
            with tarfile.open(boot_info_path) as boot_info_tar:
                systemd_blame = boot_info_tar.extractfile(
                    "./systemd_blame.txt"
                ).read()
                ...
                with tarfile.open(
                    fileobj=boot_info_tar.extractfile("./cloud-init.tar.gz")
                ) as cloud_init_logs:
                    cloud_init_log = cloud_init_logs.extractfile(
                        "cloud-init-logs-2023-03-27/cloud-init.log"
                    ).read()
                    local_time, netplan_time = extract_cloud_init_log(
                        cloud_init_log
                    )
